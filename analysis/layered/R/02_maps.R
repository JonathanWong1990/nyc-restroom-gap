...
P <- readRDS(file.path(BASE, "Build_Plan/prototype/cache/prep.rds")); X <- readRDS(file.path(LY, "cache/layers.rds"))
...
ACT <- c("BUILD" = "#c0392b", "FIX" = "#2a78d6", "EXTEND HOURS" = "#eda100", "VERIFY" = "#4a3aa7")
DESC <- c("BUILD" = "Build: no restroom within 500 m even by day",
          "FIX" = "Fix: depends on closed / failing Parks restrooms",
          "EXTEND HOURS" = "Extend hours: covered by day, not at 9pm",
          "VERIFY" = "Verify: complaints without a supply gap")
...
p <- ggplot() +
  ...
  labs(title = "Why each high-complaint neighbourhood has a problem — and what to do",
       subtitle = "Black outline = the six where the complaint excess is statistically confident (empirical Bayes P > 0.95). Grey = other NTAs.",
       caption = paste("Wednesday, posted hours, 500 m straight line to a listed-operational restroom (register dated June 2025). Precedence BUILD > FIX > EXTEND > VERIFY;",
...
ggsave(file.path(OUT, "diagnosis_map.png"), p, width = 9, height = 9.5, dpi = 160)
...
p4p <- (p1 | p2) / (p3 | p4) + plot_annotation(
  title = "Four layers: where complaints are high, and why",
...
ggsave(file.path(OUT, "layers_4panel.png"), p4p, width = 11, height = 11, dpi = 150)
...
f_html <- file.path(OUT, "diagnosis_map.html"); libdir <- file.path(OUT, "diagnosis_map_files")
if (rmarkdown::pandoc_available()) {
  ...
  saveWidget(m, f_html, selfcontained = FALSE, libdir = "diagnosis_map_files", title = "Layered diagnosis")
  h <- readLines(f_html, warn = FALSE, encoding = "UTF-8")
  inline <- function(line) {
    if (grepl('<script src="diagnosis_map_files/', line, fixed = TRUE)) {
      f <- sub('.*<script src="([^"]+)".*', "\\1", line)
      return(c("<script>", readLines(file.path(OUT, f), warn = FALSE, encoding = "UTF-8"), "</script>"))
    }
    if (grepl('<link href="diagnosis_map_files/', line, fixed = TRUE)) {
  ...
  stopifnot(!any(grepl("diagnosis_map_files/", h2, fixed = TRUE)))
