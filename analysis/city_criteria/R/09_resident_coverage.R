...
cover_plan <- function(hour = 21, ph_close = 16, levels = c(.724, .8, .9, .95, 1)) {
  ...
  g2 <- vapply(p2p, function(j) sum(rw[j][!cov[j]]), 0); n2 <- 0; hit <- rep(NA_integer_, length(levels)); curv <- numeric(0)
  lv <- levels; for (k in seq_along(lv)) if (after1 >= lv[k] - 1e-9) hit[k] <- 0L
  ...
  list(start = start, existing = n1, actions = table(factor(act, levels = c("park hours", "other hours", "repair"))),
       after_existing = after1, new_units = setNames(hit, sprintf("%g%%", 100 * levels)), curve = curv)
}
out <- list(); curves <- list()
for (sc in list(list(h = 21, ph = 16, lab = "9pm, parks close 4pm"), list(h = 21, ph = 22, lab = "9pm, parks open to 10pm"),
                list(h = 14, ph = 16, lab = "2pm (daytime)"))) {
  r <- cover_plan(sc$h, sc$ph)
  ...
  out[[sc$lab]] <- data.table(scenario = sc$lab, residents_covered_now = round(r$start, 3), existing_restrooms = r$existing,
                              ...
                              covered_after_existing = round(r$after_existing, 3), t(r$new_units))
